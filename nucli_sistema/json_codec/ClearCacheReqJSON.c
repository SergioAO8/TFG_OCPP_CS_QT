...
static struct ClearCacheReq * cJSON_GetClearCacheReqValue(const cJSON * j);
static cJSON * cJSON_CreateClearCacheReq(const struct ClearCacheReq * x);
static void cJSON_DeleteClearCacheReq(struct ClearCacheReq * x);

struct ClearCacheReq * cJSON_ParseClearCacheReq(const char * s) {
    struct ClearCacheReq * x = NULL;
    if (NULL != s) {
        cJSON * j = cJSON_Parse(s);
        if (NULL != j) {
            x = cJSON_GetClearCacheReqValue(j);
            cJSON_Delete(j);
        }
    }
    return x;
...
static struct ClearCacheReq * cJSON_GetClearCacheReqValue(const cJSON * j) {
    struct ClearCacheReq * x = NULL;
    if (NULL != j) {
        if (NULL != (x = cJSON_malloc(sizeof(struct ClearCacheReq)))) {
            memset(x, 0, sizeof(struct ClearCacheReq));
        }
    }
    return x;
}
...
char * cJSON_PrintClearCacheReq(const struct ClearCacheReq * x) {
    char * s = NULL;
    if (NULL != x) {
        cJSON * j = cJSON_CreateClearCacheReq(x);
        if (NULL != j) {
            s = cJSON_Print(j);
            cJSON_Delete(j);
        }
