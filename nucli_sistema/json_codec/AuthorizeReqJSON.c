...
static struct AuthorizeReq * cJSON_GetAuthorizeReqValue(const cJSON * j);
static cJSON * cJSON_CreateAuthorizeReq(const struct AuthorizeReq * x);
static void cJSON_DeleteAuthorizeReq(struct AuthorizeReq * x);

struct AuthorizeReq * cJSON_ParseAuthorizeReq(const char * s) {
    struct AuthorizeReq * x = NULL;
    if (NULL != s) {
        cJSON * j = cJSON_Parse(s);
        if (NULL != j) {
            x = cJSON_GetAuthorizeReqValue(j);
            cJSON_Delete(j);
        }
    }
    return x;
...
static struct AuthorizeReq * cJSON_GetAuthorizeReqValue(const cJSON * j) {
    struct AuthorizeReq * x = NULL;
    if (NULL != j) {
        if (NULL != (x = cJSON_malloc(sizeof(struct AuthorizeReq)))) {
            memset(x, 0, sizeof(struct AuthorizeReq));
...
char * cJSON_PrintAuthorizeReq(const struct AuthorizeReq * x) {
    char * s = NULL;
    if (NULL != x) {
        cJSON * j = cJSON_CreateAuthorizeReq(x);
        if (NULL != j) {
            s = cJSON_Print(j);
            cJSON_Delete(j);
        }
