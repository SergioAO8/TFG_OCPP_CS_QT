...
static struct HeartbeatReq * cJSON_GetHeartbeatReqValue(const cJSON * j);
static cJSON * cJSON_CreateHeartbeatReq(const struct HeartbeatReq * x);
static void cJSON_DeleteHeartbeatReq(struct HeartbeatReq * x);

struct HeartbeatReq * cJSON_ParseHeartbeatReq(const char * s) {
    struct HeartbeatReq * x = NULL;
    if (NULL != s) {
        cJSON * j = cJSON_Parse(s);
        if (NULL != j) {
            x = cJSON_GetHeartbeatReqValue(j);
            cJSON_Delete(j);
        }
    }
    return x;
...
static struct HeartbeatReq * cJSON_GetHeartbeatReqValue(const cJSON * j) {
    struct HeartbeatReq * x = NULL;
    if (NULL != j) {
        if (NULL != (x = cJSON_malloc(sizeof(struct HeartbeatReq)))) {
            memset(x, 0, sizeof(struct HeartbeatReq));
        }
    }
    return x;
}
...
char * cJSON_PrintHeartbeatReq(const struct HeartbeatReq * x) {
    char * s = NULL;
    if (NULL != x) {
        cJSON * j = cJSON_CreateHeartbeatReq(x);
        if (NULL != j) {
            s = cJSON_Print(j);
            cJSON_Delete(j);
        }
